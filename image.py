...
if __name__ == "__main__":
	...
	# Test script for finding the best image
	######
	#
	# 1. Find the object closest to the object of interest
	#
	# 2. Calculate the apperture flux in the template image
	...
	# Make object
	CutImage = imFits()
	CutImage._Name = "casestudy/remcut_g.fits"

	# Neccesities for running other IRAF routines
	# Can probably implement in the __init__ so that people don't do it themselves all the time
	#
	# Get skySTDEV
	CutImage.getBackgroundSTDEV()
	# Get FWHM
	CutImage.getMyMedianFWHM()

	# Find objects
	CutImage.findStars()

	# Determine GRB pixel position
	objectOfInterest = imObject()
	objectOfInterest._Name = "GRB 110918A"
	objectOfInterest._ra = 32.53894
	objectOfInterest._dec = -27.10548
	objectOfInterest._parentimage = CutImage._Name
	objectOfInterest.findLogicalPosition(write=True)
	objectOfInterest.printInfo()

	#objectOfInterest.findWorldPosition(write=True)

	# Get the closest object
	ClosestNeighbour = CutImage.findNearestNeighbour(objectOfInterest, rcirc=50, write=True)
	ClosestNeighbour[0]._Name = "Close star"
        print ""
	...
	SubtractedImage._Name = "casestudy/diff_g.fits"
	
	# Get skySTDEV and FWHM
	SubtractedImage.getBackgroundSTDEV()
	#SubtractedImage.getMyMedianFWHM()
	SubtractedImage._MEDFWHM = CutImage._MEDFWHM

	SubtractedImage.getObjectStatistics(ClosestNeighbour[0])

	# Run apperture photometry
	#
	# 1. On the close object:
	SubtractedImage.runApperturePhotometryOnObject(ClosestNeighbour[0])

	# 2. On the object of interest:
	SubtractedImage.runApperturePhotometryOnObject(objectOfInterest)
